...
const SignUp = () => {
  ...
  const verifyData = async (id, email, password) => {
    ...
    try {
      ...
      const res = await axios.post(process.env.REACT_APP_URI + 'users', {
        id: id,
        email: email,
      });
      ...
      if (res.data.status === '200') {
        ...
        res.data.status === '409' &&
        res.data.data === 'Email already exist'
      ) {
        setErrMsgEmail(res.data.data);
      } else if (
        res.data.status === '409' &&
        res.data.data === 'Id already exist'
      ) {
  ...
  const createAccount = async (fname, lname, city, street) => {
    ...
    try {
      setSpinnerState(true);
      const res = await axios.post(process.env.REACT_APP_URI + 'user', {
        ...obj,
        fname: fname,
        lname: lname,
        city: city,
      ...
      >
        ...
        <div className='btn-box'>
          ...
          {spinnerState ? (
            ...
            <button className='progress-btn' type='submit'>
