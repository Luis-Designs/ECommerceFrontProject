...
const Products = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await axios.get(process.env.REACT_APP_URI + 'category');
    ...
    (async () => {
      try {
        const res = await axios.get(
          `${process.env.REACT_APP_URI}products/${
            location.id ? location.id : categoryId
          }`
        );
  ...
  const selectedProductHandler = async (obj) => {
    // Selected product
    try {
      await axios.post(process.env.REACT_APP_URI + 'productCart', obj);
  ...
  const incrementProductCart = async (productId, request) => {
    ...
    try {
      await axios.put(process.env.REACT_APP_URI + 'productCart', {
        shoppingCartId: openCart,
        productId: productId,
        request: request,
      });
    ...
    <div className='products-container'>
      {searchStatus ? (
        <div className='search-result-box'>
          <h3>
            Search results for: &quot;{searchStatus.searchWord}&quot; (
            {searchStatus.resultsNumber})
          </h3>
        </div>
      ) : null}
      <ul>
            ...
            <li key={e.id}>
              ...
              >
                <div className='img-box'>
                  <button className='product-btn' id={e.id}>
                    <figure>
                      <Image cloudName='itamarrosenblum' publicId={e.img} />
                  ...
                  <div className='quantity-box'>
                    ...
                    <div>
                      {objProductCart.map((k) => {
                        return (
                          <span key={k.id}>
                            {k.id === e.id ? k.productCart.quantity : null}
                          </span>
                        );
                      })}
                    </div>
      ...
      {disabledBg ? <div className='disabled-bg'></div> : null}
    </div>
  );
};
