...
const Details = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await axios.post(process.env.REACT_APP_URI + 'address', {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await axios.get(process.env.REACT_APP_URI + 'dates');
  ...
  const sendOrder = async (obj) => {
    ...
    try {
      const res = await axios.post(process.env.REACT_APP_URI + 'order', obj);
  ...
  const receiptHandler = () => {
    ...
    doc.setFontSize(11);
    doc.text(
      `Bill to: ${localStorage.getItem('fname')} ${localStorage.getItem(
        'lname'
      )}`,
      45,
      50
    ...
    doc.text(
      `Shipping Address: ${shippingInfo.city}, ${shippingInfo.street}`,
      45,
      90
    );
    doc.text(
      `Delivery Date: ${new Date(shippingInfo.deliveryDate).toLocaleDateString(
        'en-GB'
      )}`,
      45,
      110
      ...
      <form className='payment-form' onSubmit={isValid}>
        <div className='city-box'>
          ...
          <div className='help-box'>
            ...
            <span>
              Double click on the 'City' field to get the defualt value
            </span>
          </div>
        ...
        <div className='street-box'>
          ...
          <div className='help-box'>
            ...
            <span>
              Double click on the 'Street' field to get the defualt value
            </span>
          </div>
          {streetErrMsg ? (
            <span className='err-msg'>{streetErrMsg}</span>
          ) : null}
        </div>
        ...
        <div className='creditCard-box'>
          ...
          {creditCardErrMsg ? (
            <div>
              <span className='err-msg'>{creditCardErrMsg}</span>
            </div>
          ) : null}
        </div>
          ...
          <div className='modal-success-box'>
            <div className='modal-success'>
              <div className='modal-success-top'>
                ...
                <p>
                  Order number is: <span>{orderId}</span>
                </p>
              </div>

              <div className='modal-success-bottom'>
                <button onClick={receiptHandler} className='download-btn'>
