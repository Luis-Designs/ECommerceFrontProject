...
const EditProduct = () => {
  ...
  const handleTextInputChange = (e) => {
    ...
    if (e.target.category.value) {
      // Category validation
      const found = arrCategory.find(
        (category) => category.name === e.target.category.value
      );
      if (found) {
  ...
  const editProduct = async () => {
    ...
    try {
      ...
      const res = await axios.put(process.env.REACT_APP_URI + 'product', obj);
    ...
    >
      ...
      <div className='category-box'>
        ...
        <div className='category-box-wraper'>
          <select name='category' id='category' defaultValue='' ref={select}>
            ...
            {arrCategory.map((e) => {
              if (e.name !== category) {
                return (
                  <option id={e.id} value={e.name} key={e.id}>
                    {e.name}
                  </option>
                );
              }
