...
const AddProduct = () => {
  ...
  const handleTextInputChange = (e) => {
    ...
    if (e.target.category.value) {
      ...
      !/[a-zA-Z].{2,}/.test(
        e.target.newCategory.value && e.target.newCategory.value.length === 0
      )
    ) {
  ...
  const addNewProduct = async (productName, category, price, img) => {
    ...
    try {
      ...
      const res = await axios.post(process.env.REACT_APP_URI + 'product', {
  ...
  const createCategory = async (productName, newCategory, price, img) => {
    // Fetch new category
    try {
      const res = await axios.post(process.env.REACT_APP_URI + 'category', {
      ...
      <div className='img-box'>
        ...
        {previewSource ? (
          ...
          <div className='placeholder-box'>
            ...
            <label htmlFor='img' className='img-btn'>
              Select Image
            </label>
          </div>
        )}
      ...
      <div className='category-box'>
        ...
        <div className='category-box-wraper'>
          ...
          >
            ...
            {arr.map((e) => {
              return (
                <option id={e.id} value={e.id} key={e.id}>
                  {e.name}
                </option>
              );
            })}
